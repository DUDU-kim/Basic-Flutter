...
class _DemoAppState extends State<DemoApp> {
  ...
  Future<void> _fetchUsers() async { //印網頁的內容
    var url = Uri.parse(ApiConfig.baseUrl + ApiConfig.getUsers);
                    ...
                    children: [
                      ...
                      Checkbox(
                        ...
                        onChanged: (bool? newValue) async {
                          // ------------------- 第 1 部分：處理「取消勾選」-------------------
                          // 如果使用者是想取消勾選，這是最簡單的情況。
                          if (newValue == false) {
                            setState(() {
                              _rememberMe = false;
                            });
                            _clearLoginInfo(); // 清除已儲存的資訊
                            return; // 直接結束，不做任何網路請求
                          }

                          // ------------------- 第 2 部分：處理「勾選」前的本地檢查 -------------------
                          // 在進行任何操作前，先檢查欄位是否為空。這是一個快速的本地檢查。
                          if (_emailController.text.isEmpty || _passwordController.text.isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('請先輸入帳戶和密碼'),
                                duration: Duration(seconds: 1),
                              ),
                            );
                            // 因為欄位是空的，所以什麼都不做，勾勾自然不會出現。
                            return;
                          }

                          // ------------------- 第 3 部分：立即更新 UI 並在背景驗證 -------------------
                          // 程式能走到這裡，代表使用者想要勾選，且欄位不是空的。
                          // 我們先「樂觀地」更新 UI，讓使用者立刻看到打勾效果。
                          setState(() {
                            _rememberMe = true;
                          });

                          // 然後，才在背景執行耗時的網路驗證。
                          // 這時使用者已經看到勾勾了，不會感覺到卡頓。
                          final result = await _loginUser();

                          // ------------------- 第 4 部分：根據驗證結果修正 UI -------------------
                          // 檢查驗證結果。如果頁面還存在 (mounted)，且登入失敗了
                          if (result != "登入成功" && mounted) {
                            // 驗證失敗，我們需要把剛剛打上的勾勾取消掉，並給予提示。
                            setState(() {
                              _rememberMe = false;
                            });
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('帳號或密碼錯誤，無法記住'),
                                duration: Duration(seconds: 1),
                              ),
                            );
                          } else if (result == "登入成功") {
                            // 只有在驗證成功後，才真正執行儲存。
                            _saveLoginInfo(_emailController.text, _passwordController.text);
                          }
                        },
                      ),
