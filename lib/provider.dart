...
import 'package:flutter/foundation.dart'; // 【新增】引入 foundation 以使用 kIsWeb
...
class SelectedAlbumProvider extends ChangeNotifier {
  ...
  Color? _nowColor;
  final Map<String, dynamic> _prefetch = {};
  ...
  Future<void> prefetchImage(BuildContext context) async{
    ...
    ImageProvider preload(dynamic pathOrUrl) {
      // 【修改】如果是 Web，直接使用 NetworkImage，不進行檔案檢查
      if (kIsWeb) {
        return NetworkImage(pathOrUrl.toString());
      }
      ...
      if (idx >= 0 && idx < _playlist.length) { // 要確保合法範圍
        ...
        if (_prefetch[cacheKey] == null) {
          ...
          if (song['image_cache'] != null) {
            ...
            // 【修改】加入 mounted 檢查，避免 context 失效報錯
            if (context.mounted) {
...
class MyPlaylistProvider with ChangeNotifier {
  ...
  Future<void> fetchAndSetPlaylists() async {
    // 構建 API 請求 URL
    final url = Uri.parse(ApiConfig.baseUrl + ApiConfig.getPlaylist);
  ...
  void renamePlaylist(String oldName, String newName) async{
    ...
    final String baseUrl = 'http://13.239.60.163'; //實機測試
    Future<void> renameSqlPlaylist(String oldName, String newName) async { // 把歌曲所存進的歌單名稱改掉(sql裡的) //songs_in_list
      final url = Uri.parse(ApiConfig.baseUrl + ApiConfig.renamePlaylist);
  ...
  Future<Map<String, int>> updateSongsNum() async {
    final url = Uri.parse(ApiConfig.baseUrl + ApiConfig.updateSongsNum);
  ...
  Future<bool> updateAddState(String title, {bool isNetWork = false}) async { //{bool isNetWork = false} 預設false寫法
    if (isNetWork) {
      // 如果 true 就走資料庫更新
      final url = Uri.parse(ApiConfig.baseUrl + ApiConfig.updateAddState);
      var response = await http.post(url,
          headers: {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36',
          },
  ...
  Future<List<dynamic>?> fetchSongList() async { //php那邊會回傳List
    final url = Uri.parse(ApiConfig.baseUrl + ApiConfig.fetchSongList);
  ...
  Future<void> addSongToPlaylist(String playlistName, Map<String, dynamic> song, bool isSelected) async {
    ...
    if (kIsWeb) {
      // 如果是 Web，直接存網址，不要下載！
      song['image_cache'] = song['image_medium'];
    } else {
      // 如果是 App，下載檔案
  ...
  Future<void> songListOperateAndDetail(String playlistName, String title, String artist, String fileUrl, String musicCache, String coverUrl, String imageMedium, String imageCache, Color color, int duration, bool isSelected) async {
    final url = Uri.parse(ApiConfig.baseUrl + ApiConfig.songListOperate);

    // 【終極防線】
    // 如果是 Web，而且要存進去的 image_cache 是以 /.tmp 或 blob: 開頭的
    // 強制把它改成 imageMedium (假設 imageMedium 是正常的 http網址)
    String cleanImageCache = imageCache;
    String cleanImageMedium = imageMedium;

    if (kIsWeb) {
      if (cleanImageCache.contains('/.tmp') || cleanImageCache.contains('blob:') || !cleanImageCache.startsWith('http')) {
        if (imageMedium.startsWith('http')) {
          cleanImageCache = imageMedium;
        } else {
          cleanImageCache = "";
        }
      }
      ...
      // 同理檢查 imageMedium
      if (cleanImageMedium.contains('/.tmp') || cleanImageMedium.contains('blob:')) {
        cleanImageMedium = "";
      }
    }
    ...
    var response = await http.post(url,
        ...
        body: {
          ...
          'image_small': coverUrl,
          'image_medium': cleanImageMedium, // 使用清洗過的
          'image_cache': cleanImageCache,   // 使用清洗過的
  ...
  Future<List<dynamic>?> fetchSongDetail(String playlistName) async {
    final url = Uri.parse(ApiConfig.baseUrl + ApiConfig.fetchSongDetail);
