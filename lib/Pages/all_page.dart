...
class DemoApp2 extends StatelessWidget {
  ...
  const DemoApp2({super.key, required this.email, required this.password});
...
class AllPage extends StatefulWidget {
  ...
  const AllPage({super.key, required this.email, required this.password});
          ...
          children: [
            ...
            Consumer<SelectedAlbumProvider>(
              builder: (context, albumProvider, child) {
                ...
                final musicCache = selectedAlbum?['music_cache']; // 已經下載到伺服器的fileUrl
                ...
                // 否則，才執行原本的「播放新歌」邏輯
                if (musicCache != null && musicCache != _fileUrl) { //在清單中按下歌曲了
                  ...
                  Future.microtask(() { //當前同步程式碼執行完畢後，立即執行
                    ...
                    String musicFileName = _fileUrl?.split("http://13.239.60.163/music_cache/")[1] as String; // 從資料庫或 API 取得的檔名
                    // print(musicFileName);
                    String encodedFileName = Uri.encodeComponent(musicFileName); //僅對檔名部分進行編碼
                    String url = "http://13.239.60.163/music_cache/$encodedFileName";
